...
specific permissions associated with each OU ACS type: per-server
permissions (Table \ref{tab:permissions-server}), each associated with
the top-level Custos server, per-group permissions (Table
\ref{tab:permissions-group}), each associated with a specific server
group, and per-object permissions (Table \ref{tab:permissions-object}),
each associated with a specific key:value object within a group. The
first three letters of a permission name indicate the type of ACS with
which it is associated: \texttt{srv}:server, \texttt{grp}:group, or
\texttt{obj}:object.
...
a lower-lever OU's ACS. For example, anyone gaining the
...
attributes are passed in each requires is not relevant. Custos treats
them as a heap of attributes and attempts to extract them from the
heap in order to satisfy the requirements of a specific ACS. The API
is RESTful and primarily stateless (individual authentication modules
are allowed to maintain state if required).
...
attributes and passed via query string as URL encoded JSON.
...
\ref{appx:messages}. That basic JSON objects used to compose Custos
requests or response include:

\noindent
\texttt{Attr} objects: JSON Dictionaries with the following keys:
...
\noindent
\texttt{Key} objects: JSON dictionaries with the following keys:
...
\item[\texttt{UUID} (Response Only)] \hfill \\ The UUID identifying
  the key
\item[\texttt{Revision} (Response Only)] \hfill \\ Used to request a
  specific key revision. If blank, the latest revision is assumed.
\item[\texttt{Status} (Response Only)] \hfill \\ The status indicating
  whether the requested operation on the Key was allowed or denied.
\end{packed_desc}

\noindent
\texttt{ACS} objects: JSON dictionaries with the following keys:
...
All API messages contain an order-agnostic list of \texttt{Attrs} JSON
...
object pass the object UUID as part of the URL. All query string
...
\texttt{``Key''} key, as well as an \texttt{ACS} JSON object (see
below) stored under the \texttt{``ACS''} key. An object update works
the same way, but only the \texttt{``Key''} key and associated JSON
object are passed. Likewise, an ACS update passes only the
\texttt{``ACS''} key and the associated JSON object. All Custos
\texttt{POST} and \texttt{PUT} requests may contain an options
\texttt{chk=} query string parameter containing a list of checksums
for the corresponding request body data.
...
%%  LocalWords:  MongoDB libcustos objs grps OU's uuid
